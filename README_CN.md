...
- 📦 **密码管理**：
  ...
  - 密码生成器工具
- 🔑 **TOTP 双重因子验证** (v1.0.4)：
  - 支持二维码图片上传、剪贴板截图粘贴、手动输入、otpauth:// 链接
  - 实时验证码生成，带倒计时（RFC 6238）
  - 支持 SHA-1/SHA-256/SHA-512 算法
  - 桌面端：列表+详情面板布局，弹窗添加
  - 移动端：卡片列表，一键复制
- 🗂️ **分组管理** (v1.0.4)：将密码组织到文件夹中（最多5个分组），紧凑 + 按钮
- 🛡️ **生物识别认证** (v1.0.3)：
  - 指纹解锁进入应用（无需主密码）
  - 可选指纹完全解锁（所有敏感操作均可用指纹）
  - 修改主密码始终需要密码输入
- ⚙️ **设置中心**：
  - 修改主密码（带密码强度验证）
  - 桌面端弹窗修改密码；移动端页面跳转 (v1.0.3)
  - 启用/禁用同步
  - 导入/导出加密备份（JSON 格式）
  - 自动锁定超时配置
  - 自定义数据目录选择
...
- **文件选择**：`file_picker`（v8.1.2）用于备份导入/导出和二维码图片选择
- **生物识别**：`local_auth`（v2.3.0）（Face ID / Touch ID / Windows Hello）
- **二维码**：`qr_code_dart_decoder`（v0.0.5）用于从图片解码二维码
- **剪贴板**：`pasteboard`（v0.3.0）用于桌面端读取剪贴板图片
- **UI**：Material 3 设计系统
...
- [x] **自动配置迁移**
- [x] **改进的错误处理**用于更新服务
- [x] **生物识别认证**（指纹解锁、完全指纹解锁）(v1.0.3)
- [x] **桌面端弹窗修改密码** (v1.0.3)
- [x] **TOTP 双重因子验证**，支持二维码扫描 (v1.0.4)
- [x] **二维码图片上传和剪贴板粘贴**用于 TOTP 设置 (v1.0.4)
- [x] **统一桌面端页面标题样式** (v1.0.4)
- [x] **桌面端列表+详情面板布局**用于 TOTP (v1.0.4)
...
### **[1.0.4]** (2025-04-09)
- ✨ TOTP 双重因子验证（RFC 6238）
- ✨ 二维码图片上传和剪贴板粘贴用于 TOTP 设置
- ✨ 统一桌面端页面标题样式
- ✨ 桌面端 TOTP 列表+详情面板布局
- ✨ 分组管理：最多5个分组，紧凑 + 按钮
- 🐛 修复指纹解锁后仍需主密码的问题
- 🐛 修复桌面端修改密码使用页面而非弹窗的问题
