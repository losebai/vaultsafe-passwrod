...
### 新增 ✨
- **TOTP 同步支持**：双重因子验证账户现在可以通过同步功能在设备间同步
  - 导出/导入备份时包含所有 TOTP 条目
  - 同步设置会将 TOTP 数据一同加密同步
- **移动端扫一扫功能**：Android/iOS 可使用相机直接扫描 TOTP 二维码
  - 添加"扫一扫"按钮，支持实时相机预览
  - 自动识别 `otpauth://` 二维码链接
  - 扫描成功后自动添加账户

### 优化 ⚡
- **数据导出完整性**：备份数据现在包含密码、分组和 TOTP 条目
- **导入结果提示**：导入后显示导入的密码数、分组数和 TOTP 数量

### 技术改进 🔧
- 添加 `mobile_scanner` 依赖用于相机扫码功能
- 修复包导入冲突（`qr_code_dart_decoder` vs `mobile_scanner`）
